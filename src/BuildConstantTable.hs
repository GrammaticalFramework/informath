...
-------------------------------------
-- TODO work more on this so that it can replace the old method
-- to experiment with it, use RunInformath -to-formalism=dedukti -new-annot <file>.dk
-- simpler annotation, consisting of lambda flattening and replacement of Dk funs by GF funs
-- the goal is to generalize annotations from lexical categories to all kinds of functions
-- so that no Dedukti identifiers need to be hard-coded in Haskell

lambdaFlatten :: DkTree a -> DkTree a
lambdaFlatten tr = case tr of
  EApp _ _ -> case splitApp tr of
    (f@(EIdent _), xs@(_:_)) -> foldl EApp f (concatMap flatten xs)
  _ -> composOp lambdaFlatten tr
 where
   flatten :: Exp -> [Exp] 
   flatten exp = case splitAbs exp of
     (xs, body) -> [EIdent (bind2ident x) | x <- xs] ++ [lambdaFlatten body]

-- annotate idents with cats and funs, just the primary
annotateDkIdentsNew :: ConstantTable -> DkTree a -> DkTree a
annotateDkIdentsNew table = annot [] . ignoreFirst where
  ...
  ignoreFirst t = case t of
    EApp _ _ -> case splitApp t of
      (EIdent f, xs@(_:_)) -> case toDrop f xs of
        xs2 -> foldl EApp (EIdent f) (map ignoreFirst xs2)
      (f, xs) -> foldl EApp (ignoreFirst f) (map ignoreFirst xs)
    _ -> composOp ignoreFirst t

  toDrop :: QIdent -> [Exp] -> [Exp]
  toDrop f xs = case M.lookup f table of
    Just entry -> case primary entry of
      (_, typ) -> case unType typ of
        (hypos, cat, _) -> case getArity cat of
	  Just n -> drop (length xs - n) xs
