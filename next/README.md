...
*The instructions given in this document work for the standard version of Informath. This will soon be updated to the code that now resides in the next/ subdirectory (the same directory as this README). In order to use it before it becomes standard, you need to add the option `-next` to each command, and some commands do not yet work with this option. You can use `-next -help` to see what exactly works. The changes are mostly code-internal refactoring, while the functionalities are still mostly the same as before.*
...
The lexicon part part of the GF grammar (files grammars/VerbalConstants*, SymbolicConstants*, UserConstants*) give verbalizations to defined constants in .dk files. The UseConstants* files can be dynamically generated with the command
```
